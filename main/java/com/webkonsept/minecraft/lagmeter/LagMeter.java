	...
	protected void handleBaseCommand(final CommandSender sender, final String[] args){
		if(args[0].equalsIgnoreCase("reload")){
			...
			if(this.permit(sender, "lagmeter.command.lagmeter.help")||this.permit(sender, "lagmeter.help")){
				int doesntHave = 0;
				this.sendMessage(sender, 0, "*           *Help for LagMeter*           *");
			...
			this.sendMessage(sender, 1, ChatColor.GOLD+"[LagMeter] "+ChatColor.RED+"Invalid sub-command. "+ChatColor.GOLD+"Try one of these:");
			this.sendMessage(sender, 0, ChatColor.GOLD+"[LagMeter] Available sub-commands: /lagmeter|lm <reload|r>|/lagmeter|lm <help|?>");
		}
	}
	...
	public boolean onCommand(final CommandSender sender, final Command command, final String commandLabel, final String[] args){
		...
		if(this.permit(sender, "lagmeter.command."+command.getName().toLowerCase())||!(sender instanceof Player)){
			if(command.getName().equalsIgnoreCase("lag")){
				...
				success = true;
				if(args.length==0){
					this.sendMessage(sender, 0, ChatColor.GOLD+"[LagMeter] Version: "+this.pdfFile.getVersion());
					this.sendMessage(sender, 0, ChatColor.GOLD+"[LagMeter] Available sub-commands: /lagmeter|lm <reload|r>|/lagmeter|lm <help|?>");
				}else
					this.handleBaseCommand(sender, args);
			}
